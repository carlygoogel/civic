...
import { Email, DashboardMetric, TopicCount } from "../utils/types";
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (emailsData) {
        ...
        const answeredEmails = emailsData.filter(
          (email) => email.status === "answered"
        ).length;
        const flaggedEmails = emailsData.filter(
          (email) => email.flagged
        ).length;
  ...
  const calculateTrend = (
    ...
    return last9Days.map((date) => {
      const dayData = data.filter((email) => email.created_at.startsWith(date));
  ...
  const calculateTopicBreakdown = (data: Email[]) => {
    ...
    // Iterate through each email and its topics array
    data.forEach((email) => {
      if (email.topics && Array.isArray(email.topics)) {
            ...
            {metric.trend && (
              <div className="h-12 mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={metric.trend.map((value, i) => ({ value, index: i }))}
                  >
                    <Line
                      type="monotone"
                      dataKey="value"
